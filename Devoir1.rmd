...
## Exercice 1
** 3 tables car deux manières de sortir de la table: la mortalité accidentelle et par maladie en brut et la mortalité réunie en net**
...
**Ne pas mettre le code, mettre l'explication plutôt en formule mathématique, voir LaTex**
```{r message = FALSE}
Racine <- 100000
...
ggplot() +
  ...
  ylab("Quotien de mortalité")
```
**Détailler effet d'âge, pas forcément besoir de parler des accidents, penser au noir et blanc**
...
**La mortalité par accident est negligeable  à 55 ans, faibles, pire **
...
```
**Calculer les pourcentages est aussi une possibilité**
...
**C'est pas la question, ils sont biaisés**
...
**Tout recalculer avec l'addition des quotients donnés 1-produit des 1-M, pour les hommes puis pour les femmes (calculer les accidents des femmes)**
```{r}
...
**Centrer les valeurs et faire des courbes, selon l'année de naissance, date, age : nuage de points**
```{r include = FALSE}
library(cowplot)
...
ggplot(DiplomeFemmeTidy) +
        geom_col_pattern(aes(x = `Age`, y = `Proportion`, pattern = Diplome, pattern_angle = Diplome, pattern_spacing = Diplome, ),fill            = 'white',
                         colour          = 'black',
                         pattern_fill    = 'black',
...
**Massificaion et universalisation du système scolaire et des études.**
# Exercice 3
